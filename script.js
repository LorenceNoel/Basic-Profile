...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Delay profile picture appearance
...
function calculateGrade() {
  ...
  if (isNaN(mark) || mark < 0 || mark > 100) {
    alert("Please put a valid grade");
    return;
  }
...
function convertTemperature() {
    ...
    // Update results
    celsiusOutput.html(`<strong>${celsius.toFixed(2)}°C</strong>`);
    kelvinOutput.html(`<strong>${kelvin.toFixed(2)}K</strong>`);

    // Add color to results
    celsiusOutput.css("color", "orange");
    kelvinOutput.css("color", "green");
}
...
let staffData = dataSet.map(staff => ({
    ...
    employeeID: staff[3],
    hireDate: staff[4],
    salary: parseInt(staff[5].replace(/[^0-9]/g, '')) // Convert "$372,000" to 372000
}));
...
// Sorting Function
function sortStaff(criteria, order) {
    staffData.sort((a, b) => {
        if (order === 'asc') {
...
// Load staff data when page is ready
